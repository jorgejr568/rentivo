...
- Pluggable cache for the billing KPI rollups (`rentivo/services/stats_cache/`) mirroring the decryption cache — `none` / `memory` / `redis` — selected by its own `RENTIVO_STATS_CACHE_BACKEND` env (plus `RENTIVO_STATS_CACHE_TTL_SECONDS`, `RENTIVO_STATS_CACHE_MAX_ENTRIES`; Redis reuses `RENTIVO_REDIS_URL`). Defaults to `memory` so the cache stays on out of the box; every backend is fail-open (#66).
