...
def _mk(key_prefix: str = "") -> RedisKVCache:
    return RedisKVCache(client=_client(), ttl_seconds=60, key_prefix=key_prefix)


def test_hashed_key_format():
    ciphertext = "enc:v1:AAAA"
    prefix = "rentivo:enc:dec:v1:"
    cache = _mk(key_prefix=prefix)
    expected = prefix + hashlib.sha256(ciphertext.encode("utf-8")).hexdigest()
    assert cache._hashed_key(ciphertext) == expected
...
def test_get_many_empty_input_returns_empty_and_skips_redis():
    client = _client()
    cache = RedisKVCache(client=client, ttl_seconds=60)
...
def test_set_many_applies_ttl():
    client = _client()
    cache = RedisKVCache(client=client, ttl_seconds=42)
    cache.set_many({"enc:v1:A": "alpha"})
    ttl = client.ttl(cache._hashed_key("enc:v1:A"))
    assert 0 < ttl <= 42
...
def test_from_url_constructs_a_real_client():
    ...
    with patch("redis.from_url") as mock_from_url:
        ...
        cache = RedisKVCache.from_url("redis://localhost:6379/0", ttl_seconds=60)
        mock_from_url.assert_called_once_with(
            "redis://localhost:6379/0",
            decode_responses=True,
        )
        assert isinstance(cache, RedisKVCache)


def test_from_url_raises_when_redis_missing():
    import rentivo.cache.redis as mod

    with patch.object(mod, "redis", None):
        with pytest.raises(ImportError, match="redis is required"):
            RedisKVCache.from_url("redis://localhost:6379/0", ttl_seconds=60)
