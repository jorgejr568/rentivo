...
status mapping, and the process-global TTL cache."""
...
class TestRollup:
    def test_empty_ids_returns_zeroed_stats_without_querying(self):
        repo = FakeBillRepo([])
        stats = BillingStatsService(repo).stats_for_ids([], today=TODAY)
    ...
    def test_status_buckets_and_expected_total(self):
        ...
        stats = BillingStatsService(repo).stats_for_ids([1, 2, 3, 4, 5], today=TODAY)
...
class TestYearToDate:
    def test_sums_every_bill_in_the_current_year(self):
        # One billing, several 2026 bills — all count toward YTD.
        repo = FakeBillRepo(
            [
        ...
        stats = BillingStatsService(repo).stats_for_ids([1], today=TODAY)
        ...
        # The table's "current" bill is the latest one regardless.
        assert stats.current[1].reference_month == "2026-05"
    ...
    def test_excludes_other_years(self):
        ...
        # but the latest bill (current) may still be the prior-year one if it is newest
        assert stats.current[1].reference_month == "2026-03"

    def test_excludes_future_months_in_the_same_year(self):
        repo = FakeBillRepo(
            [
                _summary(1, 100000, "paid", "2026-03"),
...
class TestCache:
    ...
    def test_cache_key_includes_the_ytd_window(self):
        repo = FakeBillRepo([_summary(1, 100000, "paid")])
        svc = BillingStatsService(repo)
