...
class Settings(BaseSettings):
    ...
    dashboard_cache_backend: str = "none"
    dashboard_cache_ttl_seconds: int = 30
    dashboard_cache_max_entries: int = 1000
    ...
    @field_validator("dashboard_cache_backend")
    @classmethod
    def _validate_dashboard_cache_backend(cls, v: str) -> str:
        if v not in ("none", "memory", "redis"):
            raise ValueError("RENTIVO_DASHBOARD_CACHE_BACKEND must be one of: none, memory, redis")
        return v

    @field_validator("dashboard_cache_ttl_seconds")
    @classmethod
    def _validate_dashboard_cache_ttl(cls, v: int) -> int:
        if v < 1:
            raise ValueError("RENTIVO_DASHBOARD_CACHE_TTL_SECONDS must be >= 1")
        return v

    @field_validator("dashboard_cache_max_entries")
    @classmethod
    def _validate_dashboard_cache_max_entries(cls, v: int) -> int:
        if v < 1:
            raise ValueError("RENTIVO_DASHBOARD_CACHE_MAX_ENTRIES must be >= 1")
        return v
    ...
    def _validate_redis_url_required(self) -> "Settings":
        ...
        if self.dashboard_cache_backend == "redis" and not self.redis_url:
            raise ValueError("RENTIVO_REDIS_URL is required when RENTIVO_DASHBOARD_CACHE_BACKEND=redis")
        return self
