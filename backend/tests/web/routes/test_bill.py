...
class TestBillInvoice:
    def test_invoice_local_file(self, auth_client, test_engine, tmp_path):
        ...
        # (legacy_web.services_container.get_storage), not just the legacy_web.deps re-export.
        with patch("legacy_web.services_container.get_storage", return_value=LocalStorage(str(tmp_path))):
            bill = generate_bill_in_db(test_engine, billing, tmp_path)
            response = auth_client.get(
...
class TestBillRecibo:
    ...
    def test_recibo_serves_stored_file(self, auth_client, test_engine, tmp_path):
        ...
        billing = create_billing_in_db(test_engine)
        with (
            patch("legacy_web.deps.get_storage", return_value=LocalStorage(str(tmp_path))),
            patch("legacy_web.services_container.get_storage", return_value=LocalStorage(str(tmp_path))),
        ):
            bill = generate_bill_in_db(test_engine, billing, tmp_path)
            self._mark_paid(test_engine, bill)
    ...
    def test_recibo_fires_gtm_event(self, auth_client, test_engine, tmp_path):
        ...
        with patch("legacy_web.deps.get_storage", return_value=LocalStorage(str(tmp_path))):
            ...
            self._mark_paid(test_engine, bill)
            captured: list[dict] = []
            with patch("legacy_web.routes.bill.push_event", side_effect=lambda req, ev: captured.append(ev)):
                response = auth_client.get(
                    f"/billings/{billing.uuid}/bills/{bill.uuid}/recibo",
                    follow_redirects=False,
                )
