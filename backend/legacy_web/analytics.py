...
def attach_to_context(request: Request, template_name: str, ctx: dict) -> None:
    ...
    Shared by ``legacy_web.deps.render`` and the standalone template responses in
    ``legacy_web.app`` so the dataLayer wiring lives in one place.
    """
