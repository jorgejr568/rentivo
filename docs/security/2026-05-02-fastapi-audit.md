...
|---|-------|--------|---------------|--------|-------------------|
| 1 | `ConfigDict(extra='allow')` collisions with `.dict()` / `.json()` | **None.** Only `Settings` declares an extras policy and uses `extra="ignore"` (`rentivo/settings.py:15`). All 13 `BaseModel` subclasses in `rentivo/models/` rely on the default (`extra="ignore"`). | N/A — no finding | — | None |
